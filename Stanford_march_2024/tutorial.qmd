...
**EXERCISE 2:** Unpack the skewness and kurtosis statistics using `multitool::reveal()` (once for skewness and once for kurtosis). 
...
```{r}
data02 <- read_csv('data.csv')

data02
```

When we loop over different variables, we can use `glue` syntax ({}) to refer to the variable.
Below, we use "{iv}" as a placeholder to refer to the IV, which `multitool` will then automatically replace with the correct IV.
...
full_pipeline02 <- data02 |> 
  ...
  add_model(model_desc = "lm", lm(shifting ~ {iv}))
