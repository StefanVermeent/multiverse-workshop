...
# Example 1: A multiverse of arbitrary decisions
...
## 1.5 Adding post-processing

```{r}
data |> 
  add_postprocess(postprocess_name = "skew", psych::skew(unp)) |> 
  add_postprocess(postprocess_name = "kurtosis", psych::kurtosi(unp))
```
...
```{r}
create_blueprint_graph(full_pipeline01)
```
...
#| fig-height: 20
#| fig.width: 10
curve_plot + spec_plot + var_plot + p_plot + plot_layout(ncol = 1)
```
