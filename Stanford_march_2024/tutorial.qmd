...
## 1.2. Adding variables

It is also possible to include multiple independent or dependent variables to loop over. In our case, we have two IVs: exposure to violence (`vio`) and exposure to unpredictability (`unp`). We want to apply the exact same multiverse to both, so adding them at the same time is convenient:
